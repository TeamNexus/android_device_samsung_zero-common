...
static void power_apply_profile(struct power_profile data) {
	...
	if (data.cpu.cluster0.govoverride.governor != "")
	{
		shouldOverride = true;
	...
	// governor override
	shouldOverride = false;
	if (data.cpu.cluster1.govoverride.governor != "")
	{
		shouldOverride = true;
		
		// apply cpu-governor override for cluster0
		file_write(POWER_CLUSTER1_GOVERNOR, data.cpu.cluster0.govoverride.governor);
	}
